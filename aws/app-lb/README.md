...
| <a name="input_alb_type_internal"></a> [alb\_type\_internal](#input\_alb\_type\_internal) | Boolean to specify whether type of ALB is internal. True == internal; false == external. | `bool` | `false` | no |
...
| <a name="input_https_health_check"></a> [https\_health\_check](#input\_https\_health\_check) | Map describing HTTPS Health Check settings - including endpoint (default /) and port (default 443). Applied for external LBs. | `map(string)` | <pre>{<br>  "healthy_threshold": "3",<br>  "interval": "20",<br>  "matcher": "200",<br>  "path": "/",<br>  "port": "443",<br>  "protocol": "HTTPS",<br>  "timeout": "10",<br>  "unhealthy_threshold": "2"<br>}</pre> | no |
